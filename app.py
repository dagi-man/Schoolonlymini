...
def connect():
    ...
    # Use DELETE journal mode for better reliability on ephemeral filesystems
    # (WAL can leave -shm/-wal files that cause "disk image is malformed" after unclean shutdown)
...
if __name__ == "__main__":
    import sys
    # Ensure prints appear immediately (important on Render / containers)
    try:
    ...
    try:
        ...
        print(f"ensure_registration_codes warning: {e}")
    ...
    print("━" * 50)
    # threaded=False is safer with SQLite on single-process hosts
